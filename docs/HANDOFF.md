...
- **CLI / Homebrew / deploy residual (one list — do not oversell):**
  ...
  5. **queria-cli config (on branch `feat/queria-cli-config` until merged)** — multi-profile `~/.config/queria/config.toml`; **TUI only** (`queria-cli config`, no set/list subcommands); MCP install from TUI; `index-here` resolve env > file. Spec: [`archive/superpowers/specs/2026-07-21-queria-cli-config-design.md`](./archive/superpowers/specs/2026-07-21-queria-cli-config-design.md). Not required for Daily MCP.
