...
- **CLI / Homebrew / deploy residual (one list — do not oversell):**
  ...
  5. **queria-cli config (on branch `feat/queria-cli-config` until merged)** — multi-profile `~/.config/queria/config.toml`, TUI `queria-cli config`, subcommands + `config mcp`, `index-here` resolve order file/env. Spec: [`archive/superpowers/specs/2026-07-21-queria-cli-config-design.md`](./archive/superpowers/specs/2026-07-21-queria-cli-config-design.md). Not required for Daily MCP.
