...
- **CLI / Homebrew / deploy residual (one list — do not oversell):**
  ...
  5. **queria-cli config (on `main`, package `0.2.0`)** — multi-profile `~/.config/queria/config.toml`; **TUI only** (`queria-cli config`); MCP install from TUI; `index-here` resolve env > file. Residual: re-tag **`cli-v0.2.0`** so Download/Brew assets include the TUI (push `main` does not publish CLI). Spec: [`archive/superpowers/specs/2026-07-21-queria-cli-config-design.md`](./archive/superpowers/specs/2026-07-21-queria-cli-config-design.md). Not required for Daily MCP.
